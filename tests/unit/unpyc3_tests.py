class TestClass:
    ...
    def test_func2():
        if a and ((b and c and d) or e or f) and g:
            g()
        if a or (b and (c1 or c2) and d) or e:
            g()
        if a and b or c:
            g()
        if a or b and c:
    ...
    if len(sys.argv) == 1:
        ...
        for k, func in TestClass.__dict__.items():
            if isinstance(func, types.FunctionType):
                ...
                source = str(
                    code.get_suite(include_declarations=False, look_for_docstring=True)
                )
        ...
        diff = difflib.unified_diff(
            str(code), str(code2), fromfile="original", tofile="converted"
        )
